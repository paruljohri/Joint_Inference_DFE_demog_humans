...
#make sure that only the correct chromosome is being read
f_fa = open("/scratch/pjohri1/BgsDfeDemo_Human/HumanData/RefGenome/GRCh37_latest_genomic.fna", 'r')
line_start_coord = 0
check = 0
for line in f_fa:
    line1 = line.strip('\n')
    if line1[0] == ">":
        line2 = line1.split()
        if line2[0] == ">" + d_chrom_ncbi[s_chrom]:
            check = 1
        else:
            check = 0
    else:
        if check == 1:#belongs to relevant chromosome:
            if line_start_coord == 0:
                line_start_coord = 1
            else:
                line_start_coord += 80
            line_end_coord = line_start_coord + 80
            for s_exon in l_exons:
...
#write the results:
result_exon = open("/home/pjohri1/BgsDfeDemo_Human/Humans/Exons/divergence/GRCH37/exon/" + s_chrom + ".fa", 'w+')
result_5p = open("/home/pjohri1/BgsDfeDemo_Human/Humans/Exons/divergence/GRCH37/5p/" + s_chrom + ".fa", 'w+')
result_3p = open("/home/pjohri1/BgsDfeDemo_Human/Humans/Exons/divergence/GRCH37/3p/" + s_chrom + ".fa", 'w+')
for s_exon in l_exons:
    result_exon.write(">" + s_exon +"_exon_grch37" + '\n')
    result_exon.write(d_anc_exon[s_exon] + '\n')
    #writing intergenic sequences with noncoding and then exonic                
    if d_strand[s_exon] == "+":                                                 
        result_5p.write(">" + s_exon +"_5p_grch37" + '\n')                            
        result_5p.write(d_anc_5p[s_exon] + '\n') #written in forward               
        result_3p.write(">" + s_exon +"_3p_grch37" + '\n')                            
