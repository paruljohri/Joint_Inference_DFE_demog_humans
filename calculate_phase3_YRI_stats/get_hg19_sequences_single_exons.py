...
#os.system("gzip -d " + s_chrom + ".fa.gz")
f_fa = open("/scratch/pjohri1/BgsDfeDemo_Human/HumanData/RefGenome/hg19_ucsc/" + s_chrom + ".fa", 'r')
line_start_coord = 0
for line in f_fa:
    line1 = line.strip('\n')
    if line1[0] != ">":
        if line_start_coord == 0:
            ...
            line_start_coord += 50
        line_end_coord = line_start_coord + 50
        for s_exon in l_exons:
...
#write the results:
result_exon = open("/home/pjohri1/BgsDfeDemo_Human/Humans/Exons/divergence/HG19/exon/" + s_chrom + ".fa", 'w+')
result_5p = open("/home/pjohri1/BgsDfeDemo_Human/Humans/Exons/divergence/HG19/5p/" + s_chrom + ".fa", 'w+')
result_3p = open("/home/pjohri1/BgsDfeDemo_Human/Humans/Exons/divergence/HG19/3p/" + s_chrom + ".fa", 'w+')
for s_exon in l_exons:
    result_exon.write(">" + s_exon +"_exon_hg19" + '\n')
    result_exon.write(d_anc_exon[s_exon] + '\n')
    if d_strand[s_exon] == "+":                                                 
        result_5p.write(">" + s_exon +"_5p_hg19" + '\n')                            
        result_5p.write(d_anc_5p[s_exon] + '\n') #written in forward               
        result_3p.write(">" + s_exon +"_3p_hg19" + '\n')                            
