...
#This is to calcualte stats only for a particular set of genes
...
parser.add_argument('-category', dest = 'category', action='store', nargs = 1, type = str, help = 'low_rec_low_div/low_rec_high_div/high_rec_low_div/high_rec_high_div')
...
f_par = open("/home/pjohri1/BgsDfeDemo_Human/demo_dfe_SingExon_human_logunif_parameters_v2.txt", 'r')
d_col = {}
for line in f_par:
    line1 = line.strip('\n')
    line2 = line1.split('\t')
...
#read the genes in the category
l_genes = []
f_cat = open("/home/pjohri1/BgsDfeDemo_Human/Humans/single_exon_" + s_category + ".txt", 'r')
for line in f_cat:
    line1 = line.strip('\n')
    line2 = line1.split('\t')
    if line2[0] != "exon":
        l_genes.append(line2[1])
f_cat.close()

#go through selected simulation replicates and read data into pylibseq format
#addin the option of ignoring some files if they don't exist
#geneID = 1
s_absent = 0
for geneID in l_genes:
    #define windows specific to the gene
    s_coding_size = int(d_codingsize[geneID])
    s_numbp50 = int(d_numbp50[geneID])
    ...
    while repID <= num_rep:
        if repID > 0:
        #try:
            print ("Reading file: sim" + str(simID) + "_" + str(geneID) + "_rep" + str(repID))
            f_subs = open("/scratch/pjohri1/BgsDfeDemo_Human/" + folder + "/" + subfolder + "/sim" + str(simID) + "_" + str(geneID) + "_rep" + str(repID) + ".fixed", 'r')
            d_subs = {}
            ...
            #reading ms file:
            f_ms = open("/scratch/pjohri1/BgsDfeDemo_Human/" + folder + "/" + subfolder + "/sim" + str(simID) + "_" + str(geneID) + "_rep" + str(repID) + ".ms", 'r')
            ...
            #write out stats:
            for win in l_windows:
                result.write(str(geneID) + "_rep" + str(repID) + '\t' + win + '\t' + str(round((d_end[win]-d_start[win])*float(chr_len))) + '\t' + str(d_filt_sites[str(geneID)][win]) + '\t' + print_5_dig(d_ps[win].thetapi()) + '\t' + print_5_dig(d_ps[win].thetaw()) + '\t' + print_5_dig(d_ps[win].thetah()) + '\t' + print_5_dig(d_ps[win].hprime()) + '\t' + print_5_dig(d_ps[win].tajimasd()) + '\t' + str(d_ps[win].numexternalmutations()) + '\t' + print_5_dig(d_ps[win].hapdiv()))
                #write out LD stats:
