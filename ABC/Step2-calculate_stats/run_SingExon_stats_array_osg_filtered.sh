...
python filter_ms_phase3_data.py ${simID} ${intergenic} "demo_dfe_SingExon_human_v2"

#get stats:
cd /home/pjohri1/BgsDfeDemo_Human/programsABC

if [ ! -f "/scratch/pjohri1/BgsDfeDemo_Human/demo_dfe_SingExon_human_v2_stats_filtered_${intergenic}/sim"$simID"_bigwindow.stats" ]
then
    echo "Calculating stats for neutral, linked, and functional regions"
    python statistics_bigwindow_pylibseq_SingExon_osg_human_filtered.py -folder demo_dfe_SingExon_human_v2 -outFolder demo_dfe_SingExon_human_v2_stats_filtered_${intergenic} -interRegion ${intergenic} -simID ${simID} -numGenes 465 -numRep 1
fi

Rscript ./get_final_statistics_SingExon_filtered.R demo_dfe_SingExon_human_v2_stats_filtered_${intergenic} ${simID}
