...
cd /scratch/pjohri1/BgsDfeDemo_Human/ModelViolations_aa
unzip sim${simID}.zip

#filter the .ms and .fixed files:
cd /home/pjohri1/BgsDfeDemo_Human/programs_model_violations
python filter_ms_phase3_data.py ${simID} ${intergenic} "ModelViolations_aa"

#get stats:
cd /home/pjohri1/BgsDfeDemo_Human/programs_model_violations

if [ ! -f "/scratch/pjohri1/BgsDfeDemo_Human/ModelViolations_aa_stats_filtered_${intergenic}/sim"$simID"_bigwindow.stats" ]
then
    echo "Calculating stats for neutral, linked, and functional regions"
    python statistics_bigwindow_pylibseq_SingExon_osg_human_filtered.py -folder ModelViolations_aa -outFolder ModelViolations_aa_stats_filtered_${intergenic} -interRegion ${intergenic} -simID ${simID} -numGenes 465 -numRep 1
fi

cd /home/pjohri1/BgsDfeDemo_Human/programsABC
Rscript ./get_final_statistics_SingExon_filtered.R ModelViolations_aa_stats_filtered_${intergenic} ${simID}
